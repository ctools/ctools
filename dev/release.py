...
def release_menu(branch='release'):
    ...
    print("")
    if confirm("Step 2: Current ctools version is '%s'. Do you want to "
    ...
    if confirm("Step 4: Commit changes?"):
        ...
        message = "ctools package version set to '%s' and libtool version "\
...
def package_version_menu():
    ...
    while looping:
        ...
        waiting = True
        while waiting:
            version = str(raw_input("Current ctools version is '%s'. Please "
                                    "enter new ctools version: " % current))
            if version == "q":
                sys.exit()
            if check_version(version) == False:
                print("*** Invalid ctools version. "
        ...
        if confirm("Change version to '%s'?" % version):
            ...
            print("ctools version changed to '%s'" % (version))
