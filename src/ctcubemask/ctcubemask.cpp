 ...
 ***************************************************************************/
ctcubemask::ctcubemask(void) : ctobservation(CTCUBEMASK_NAME, VERSION)
{
    // Initialise members
    init_members();
...
ctcubemask::ctcubemask(const GObservations& obs) :
            ctobservation(CTCUBEMASK_NAME, VERSION, obs)
{
    // Initialise members
    init_members();
...
ctcubemask::ctcubemask(int argc, char *argv[]) : 
            ctobservation(CTCUBEMASK_NAME, VERSION, argc, argv)
{
    // Initialise members
    init_members();
...
void ctcubemask::apply_mask(GCTAObservation* obs)
{
    ...
    if (cube != NULL) {
        ...
        if (roi.is_valid()) {
            GSkyRegionCircle circle(roi.centre().dir(), roi.radius());
            for (int i = e_idx1; i <= e_idx2; ++i) {
                for (int pixel = 0; pixel < npix; ++pixel) {
                    GSkyDir dir = map.inx2dir(pixel);
                    if (!circle.contains(dir)) {
                        map(pixel,i) = -1.0;
                    }
                }
            }
            ...
            log_value(NORMAL, "Selected RoI",
                      "RA="+gammalib::str(circle.centre().ra_deg())+" deg, "+
