...
void ctselect::get_parameters(void)
{
    ...
    // Signal whether the RoI selection should be enforced even when leading
    // to invalid RoI
    m_forcesel = (*this)["forcesel"].boolean();
                             ...
                             const std::string& gtiname)
{
    ...
    // Create boolean to remember if RoI selection is enforced regardless of
    // its validity
        ...
        if (roi_radius > 0.0) {
            ...
            // to cope with numerical precision limitations
                ...
                if (m_forcesel) {
                    enforced_roi = true;
                    std::string msg = \
                        "WARNING: Enforced RoI selection leading to inconsis" \
                        "tent RoI.\n" \
                        "         RoI of data: Centre(RA,DEC)=("+ \
                        gammalib::str(ra)+", "+gammalib::str(dec)+") deg, " \
                        "Radius="+gammalib::str(rad)+" deg.\n" \
                        "         Requested RoI: Centre(RA,DEC)=("+ \
                        gammalib::str(roi_centre.ra_deg())+", "+ \
                        gammalib::str(roi_centre.dec_deg())+") deg, " \
                        "Radius="+gammalib::str(roi_radius)+" deg.\n" \
                        "         The resulting event list file is not usable " \
