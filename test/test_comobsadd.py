...
class Test(test):
    ...
    def _check_result(self, filename, foldername, nobs=1):
        ...
        # Check DRW file
        fname = '%s/com_000750-001000keV_drw.fits' % foldername
        exists  = os.path.isfile(fname)
        self.test_assert(exists, 'Check that DRW file "%s" exists' % fname)
        if exists:
            drw = gammalib.GCOMDri(fname)
            self.test_value(drw.size(), 10000, 'Check DRW size')
            self.test_value(drw.nchi(), 20, 'Check DRW Chi size')
            self.test_value(drw.npsi(), 20, 'Check DRW Psi size')
